...
Defer to the global skill-routing layer — `~/.claude/rules/skill-routing.md`
(the AUTO / HARD-GATE / ASK / manual-only tiers, ASK-by-default: announce intent
+ confirm before invoking, plus the `auto-invoke skills` override) and the
topic-grouped when-to-use routing files in `~/.claude/skills-routing/`. ollama-marshal has no
repo-specific routing overrides, so the global layer governs; this section is a
pointer only (kept so tooling sees routing is configured).
